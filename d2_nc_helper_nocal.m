...
% depending on your matlab version you may need to do the matlab path-setting
% below once, or everytime; adjust paths to match your machine :)

% Set matlab path to include tools
% nominally available at https://www.soundtags.org/dtags/dtag-toolbox/
% this project's version at:
% https://github.com/stacyderuiter/socal-atn-dac
addpath(genpath('dtagtools'))
...
% or our repo, https://github.com/stacyderuiter/socal-atn-dac
addpath(genpath('animaltagtools/matlab'))
savepath

%#########################################################################
% INFORMATION SPECIFIC TO EACH DEPLOYMENT
...
ncname = 'bw11_214b'; % if nc file name and depid are to be the same 
...
% tag, if you keep the same directory structure.
...
settagpath('cal', [base_dir ncname '/TAG data'], ... % where *cal.mat files are
    'prh', [base_dir ncname '/TAG data'], ...
    'audio', [base_dir ncname '/TAG data/dtg']); % where *prh.mat files are
...
% doing this now in sound_archive [ct,ref_time,fs,fn] = d2getcues(recdir, ncname) ;
SA = socal_sound_archive(ncname, recdir); % will spit out a lot of info to screen, sorry
